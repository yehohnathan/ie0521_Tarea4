...
def parse_trace_file(file_path):
    ...
    cache_associativity_pattern = r'\+ Variación de asociatividad del caché: (\d+) \+'
    ...
    for i in range(0, len(traces), 2):
        ...
        cache_associativity = re.findall(cache_associativity_pattern, trace_data)
        results = re.findall(results_pattern, trace_data)
        
        for size, (misses, miss_rate) in zip(cache_associativity, results):
            data.append([trace_name, int(size), int(misses), float(miss_rate)])
    
    return pd.DataFrame(data, columns=['Trace', 'Cache Associativity', 'Total Misses', 'Miss Rate'])

# Parseamos el archivo
file_path = 'Efecto_asociatividad_cache.txt'  # Reemplace con el nombre de su archivo
...
cache_associativity = data['Cache Associativity'].unique()
mean_miss_rates = []

for size in cache_associativity:
    rates = data[data['Cache Associativity'] == size]['Miss Rate']
    mean_miss_rate = gmean(rates)
    mean_miss_rates.append([size, mean_miss_rate])

# Convertimos a DataFrame para facilitar el manejo
mean_miss_rates_df = pd.DataFrame(mean_miss_rates, columns=['Cache Associativity', 'Mean Miss Rate'])
...
mean_miss_rates_df.to_csv(os.path.join(output_dir, 'mean_miss_rates_associativity.csv'), index=False)
data.to_csv(os.path.join(output_dir, 'trace_data_associativity.csv'), index=False)

# Paso 4: Graficar los Datos

# Graficamos los datos
plt.figure(figsize=(10, 6))
plt.plot(mean_miss_rates_df['Cache Associativity'], mean_miss_rates_df['Mean Miss Rate'], marker='o')
plt.title(' Tamaño de Asociatividad del Caché vs Miss Rate Total Promedio')
plt.xlabel('Tamaño de Asociatividad del Caché')
plt.ylabel('Miss Rate Total Promedio (%)')
plt.xticks(mean_miss_rates_df['Cache Associativity'])
plt.yticks(mean_miss_rates_df['Mean Miss Rate'])
plt.grid(True)



plt.savefig(os.path.join(output_dir, 'miss_rate_vs_cache_associativity.png'))
plt.show()
...
def create_latex_table(data, filename):
    ...
    cache_associativity = data['Cache Associativity'].unique()
    for size in cache_associativity:
        cache_data = data[data['Cache Associativity'] == size]
...
create_latex_table(data, os.path.join(output_dir, 'trace_data_associativity.tex'))

# Paso 6: Análisis del Trace "470.lbm-1274B.trace.txt.gz"

# Filtramos los datos para el trace específico
specific_trace = '470.lbm-1274B.trace.txt.gz'
...
if specific_trace_data.empty:
    ...
    plt.figure(figsize=(10, 6))
    plt.plot(specific_trace_data['Cache Associativity'], specific_trace_data['Miss Rate'], marker='o')
    plt.title(f'Tamaño de Asociatividad del Caché vs Miss Rate para {specific_trace}')
    plt.xlabel('Tamaño de Asociatividad del Caché')
    plt.ylabel('Miss Rate (%)')
    plt.xticks(specific_trace_data['Cache Associativity'])
    plt.yticks(specific_trace_data['Miss Rate'])
    plt.grid(True)
    
    plt.xticks(specific_trace_data['Cache Associativity'])

    plt.savefig(os.path.join(output_dir, f'miss_rate_associativity_{specific_trace}.png'))
    plt.show()
    
    # Guardamos la tabla para el trace específico
    create_latex_table(specific_trace_data, os.path.join(output_dir, 'specific_trace_data_associativity.tex'))
