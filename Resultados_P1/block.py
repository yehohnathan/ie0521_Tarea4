...
def parse_trace_file(file_path):
    ...
    cache_block_pattern = r'\+ Variación de tamaño del bloque en el caché: (\d+) \+'
    ...
    for i in range(0, len(traces), 2):
        ...
        cache_block = re.findall(cache_block_pattern, trace_data)
        results = re.findall(results_pattern, trace_data)
        
        for size, (misses, miss_rate) in zip(cache_block, results):
            data.append([trace_name, int(size), int(misses), float(miss_rate)])
    
    return pd.DataFrame(data, columns=['Trace', 'Cache Block', 'Total Misses', 'Miss Rate'])

# Parseamos el archivo
file_path = 'Efecto_tamaño_bloque_cache.txt'  # Reemplace con el nombre de su archivo
...
cache_block = data['Cache Block'].unique()
mean_miss_rates = []

for size in cache_block:
    rates = data[data['Cache Block'] == size]['Miss Rate']
    mean_miss_rate = gmean(rates)
    mean_miss_rates.append([size, mean_miss_rate])

# Convertimos a DataFrame para facilitar el manejo
mean_miss_rates_df = pd.DataFrame(mean_miss_rates, columns=['Cache Block', 'Mean Miss Rate'])
...
mean_miss_rates_df.to_csv(os.path.join(output_dir, 'mean_miss_rates_block.csv'), index=False)
data.to_csv(os.path.join(output_dir, 'trace_data_block.csv'), index=False)

# Paso 4: Graficar los Datos

# Graficamos los datos
plt.figure(figsize=(10, 6))
plt.plot(mean_miss_rates_df['Cache Block'], mean_miss_rates_df['Mean Miss Rate'], marker='o')
plt.title(' Tamaño del Bloque del Caché vs Miss Rate Total Promedio')
plt.xlabel('Tamaño del Bloque del Caché')
plt.ylabel('Miss Rate Total Promedio (%)')
plt.xticks(mean_miss_rates_df['Cache Block'])
plt.yticks(mean_miss_rates_df['Mean Miss Rate'])
plt.grid(True)



plt.savefig(os.path.join(output_dir, 'miss_rate_vs_cache_block.png'))
plt.show()
...
def create_latex_table(data, filename):
    ...
    cache_block = data['Cache Block'].unique()
    for size in cache_block:
        cache_data = data[data['Cache Block'] == size]
...
create_latex_table(data, os.path.join(output_dir, 'trace_data_block.tex'))

# Paso 6: Análisis del Trace "401.bzip2-226B.trace.txt.gz"

# Filtramos los datos para el trace específico
specific_trace = '401.bzip2-226B.trace.txt.gz'
...
if specific_trace_data.empty:
    ...
    plt.figure(figsize=(10, 6))
    plt.plot(specific_trace_data['Cache Block'], specific_trace_data['Miss Rate'], marker='o')
    plt.title(f'Tamaño del Bloque del Caché vs Miss Rate para {specific_trace}')
    plt.xlabel('Tamaño del Bloque del Caché')
    plt.ylabel('Miss Rate (%)')
    plt.xticks(specific_trace_data['Cache Block'])
    plt.yticks(specific_trace_data['Miss Rate'])
    plt.grid(True)
    
    plt.xticks(specific_trace_data['Cache Block'])

    plt.savefig(os.path.join(output_dir, f'miss_rate__block{specific_trace}.png'))
    plt.show()
    
    # Guardamos la tabla para el trace específico
    create_latex_table(specific_trace_data, os.path.join(output_dir, 'specific_trace_data_block.tex'))
